...
import {
  ...
  startOfWeek, isWithinInterval, differenceInDays, subMonths
} from 'date-fns';
          ...
          {filteredTransactions.length === 0 ? (
            ...
            <div className="space-y-2">
              {filteredTransactions.map(transaction => {
                const fireInfo = getTransactionFireInfo(
                  transaction,
                ...
                return (
                  <div
                    key={transaction.id}
                    className="flex items-center gap-3 p-3 rounded-xl bg-secondary/50 touch-scale transaction-item group"
                  >
                    <div
                      className="w-11 h-11 rounded-xl flex items-center justify-center transition-all duration-200 group-hover:scale-110"
                      style={{
                        backgroundColor: `${cat?.color}15`,
                        boxShadow: `0 4px 12px -4px ${cat?.color}40`,
                      }}
                    >
                      <IconComponent
                        size={20}
